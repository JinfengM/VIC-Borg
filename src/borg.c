...
static char* BORG_Debug_name = "VIC MODEL";
...
struct BORG_Problem_t {
	...
	double* epsilons;
	const char** names;
	void (*function)(double*, double*, double*,int,char**);
};
...
struct BORG_Algorithm_t {
	...
	//父代创建时，population种群中选择k-1个，archive中选择1个，总共k个构成父代
...
BORG_Problem BORG_Problem_create(
		...
		int numberOfObjectives,
		int numberOfConstraints,
		void (*function)(double*, double*, double*,int argc,char* argv[])) {
...
void BORG_Solution_evaluate(BORG_Solution solution,int argc,char* argv[]) {
	BORG_Validate_pointer(solution);
	...
	//输入variables,返回objectives & constrains
	solution->problem->function(solution->variables, solution->objectives, solution->constraints,argc,argv);
...
//清空方案，重置容量大小为newCapacity，仅是分配空间大小，并未放置真正solution
...
void BORG_Archive_add(BORG_Archive archive, BORG_Solution solution) {
	...
	DOMINATED = 2
	*/
	//如果存档存在entry,循环对比 solution,返回flag标识
	while (entry) {
		//循环对比
	...
	//根据solution 创建entry,目前还没有存储到存档中
...
//根据种群大小，动态调整竞技尺寸(selectionRatio*population->size)
...
//谁在调用注入？何时调用注入？
void BORG_Algorithm_injection(BORG_Algorithm algorithm, int size,int argc,char* argv[]) {
	BORG_Validate_pointer(algorithm);
	...
	while (remaining > 0) {
		...
		for (i=0; i<numberOfOffspring; i++) {
			...
			BORG_Solution_evaluate(offspring[i],argc,argv);
...
void BORG_Algorithm_restart(BORG_Algorithm algorithm,int argc,char* argv[]) {
	BORG_Validate_pointer(algorithm);
	...
	//曾经在哪里看到了BORG_Algorithm_inject?还是错觉
	BORG_Algorithm_injection(algorithm, newPopulationSize,argc,argv);
...
void BORG_Algorithm_step(BORG_Algorithm algorithm,int argc,char* argv[]) {
	fprintf(stderr,"                                                                 \n");
	fprintf(stderr,"                                                                 \n");
	fprintf(stderr,"current numberOfEvaluations is: %d\n",algorithm->numberOfEvaluations);
	fprintf(stderr,"                                                                 \n");
	fprintf(stderr,"                                                                 \n");
	BORG_Validate_pointer(algorithm);
	...
	if ((algorithm->numberOfEvaluations-algorithm->evaluationsAtLastCheck) >= algorithm->windowSize) {
		if (BORG_Algorithm_check(algorithm)) {
			...
			BORG_Algorithm_restart(algorithm,argc,argv);
	...
	if (algorithm->numberOfEvaluations == 0) {
		...
		for (i=0; i<population->capacity; i++) {
			...
			/*
			建议逻辑 
			1.创建population->capacity个task，逐个执行
			2.返回population->capacity个结果
			3.循环population->capacity，更新到population
			3.循环population->capacity，更新到archive
			*/
			BORG_Solution_evaluate(solution,argc,argv);
			//填充种群
...
BORG_Archive BORG_Algorithm_run(BORG_Problem problem, int maxEvaluations,int argc,char* argv[]) {
	BORG_Validate_pointer(problem);
	...
	while (BORG_Algorithm_get_nfe(algorithm) < maxEvaluations) {
		BORG_Algorithm_step(algorithm,argc,argv);
	}
