...
c     filenames allowed a maximum of 5 decimal places
      ...
      WRITE(JICHAR(1),'(F9.4)')JLOC
      WRITE(JICHAR(2),'(F9.4)')ILOC
...
C PI PJ为站点位置
     ...
     $  IROW,ICOL,XC,YC,SIZE,NAME5)

      IMPLICIT NONE
C add XC,YC,SIZE,NAME5
      CHARACTER*5  NAME5
	  REAL        JLOC, ILOC
      REAL XC, YC, SIZE
...
C****** CATCHMENTS ***************************************
      open(111, file = NAME5//'.geo', STATUS='UNKNOWN')
	  CLOSE(111,STATUS='DELETE')
	  open(111, file = NAME5//'.geo', status='new')
      NO_OF_BOX = 0
...
c calculate geo location			   
			   ILOC=XC + (ICOL-I)*SIZE + SIZE/2.0
               JLOC=YC + J*SIZE - SIZE/2.0
			   write(111,'(2F9.4)') ILOC,JLOC
               GOTO 310
            ELSE 
